function getKeywordData(hub, clusters) {
  const kw = {
  ...
  if (kw.pageTitle.length < 60) {
    kw.pageTitle += ' | Kitchen Deals';
  }

  const entityType = hub.entityType;
  if (entityType === 'Brand') {
    const brandName = hub.name;
    const brandRetailers = clusters.retailers.slice(0, 5);
    kw.subHeaders = [
    ...
    kw.sections = [
      {
        ...
        links: [
          { href: '/kitchen-deals', label: 'Browse all kitchen deals' },
        ],
      },
      {
      ...
      },
      {
        ...
        links: [
          { href: '/kitchen-appliance-package-deals', label: 'See appliance package deals' },
        ],
      },
      {
